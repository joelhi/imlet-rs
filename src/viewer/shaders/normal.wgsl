...
fn fs_main(in: VertexOutput, @builtin(front_facing) front_face: bool) -> @location(0) vec4<f32> {
    ...
    let clamp_min: f32 = 0.25;
    let clamp_max: f32 = 1.0;
    ...
    var shade: f32 = clamp_min + f32(step) * step_size + step_size * 0.5;

    // Invert the color for back-facing fragments
    if !front_face {
        shade = clamp_max - shade + clamp_min; // Reflect the color around the midpoint of the clamp range
    }
